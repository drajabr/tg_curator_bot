...
from ._flow import _FlowMixin
from ._history import _HistoryMixin
from ._rules import _RulesMixin
from ._screens import _ScreensMixin
from ._sources import _SourcesMixin
...
class TelegramFeedBot(_FlowMixin, _HistoryMixin, _SourcesMixin, _ScreensMixin, _RulesMixin):
    ...
    def _group_state(self, state: Dict[str, Any], group_id: int) -> Dict[str, Any]:
        groups = state.get("groups", {})
    ...
    async def _handle_reapply_rule_callback(self, callback_query) -> None:
        ...
        await callback_query.answer(self._flow_text("common.applying"))
        await self._safe_edit_message_text(
            callback_query.message,
            self._flow_text("rule.reapply_in_progress"),
        )
        result = await self._retroapply_filters_in_range(group_id, since)
        failed_suffix = self._flow_failed_suffix(result.get("skipped", 0))
        await self._send_flow_acknowledge(
